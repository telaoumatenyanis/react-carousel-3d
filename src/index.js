...
export class Carousel extends React.Component {
  ...
  componentDidMount() {
    ...
    this.setState((prevState, props) => {
      return { slides, slideTotal: this.props.slides.length - 1 };
    });
    if (this.state.slideCurrent === -1)
      setTimeout(() => {
        this.slideRight();
        height: document.getElementsByClassName("slider-single")[0]
          .clientHeight;
      }, 500);
  }
  ...
  slideRight() {
    ...
    if (document.getElementsByClassName("slider-single active").length > 0) {
      setTimeout(() => {
        let height = document.getElementsByClassName("slider-single active")[0]
          .clientHeight;
        this.setState((prevState, props) => {
